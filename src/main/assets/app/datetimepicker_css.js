...
function RenderCssCal(bNewCal)
{
	...
	funcCalback += " var CalId = document.getElementById(id);if (datum=== 'undefined') { var d = new Date(); datum = d.getDate() + '/' +(d.getMonth()+1) + '/' + d.getFullYear() ; } window.calDatum=datum; var cc=datum;";
	funcCalback += " if(Cal.ShowTime){";
	funcCalback += " cc+=' '+Cal.getShowHour()+':'+Cal.Minutes;";
	funcCalback += " if (Cal.ShowSeconds)  cc+=':'+Cal.Seconds;";
	funcCalback += " if (TimeMode === 12)  cc+=''+Cal.getShowAMorPM();}";
	funcCalback += " CalId.value=cc; /*datetools_milliToDate(datetools_dateToMilli(cc))*/;";
	...
	if (!winCal)
	{
		...
		span.style.left = (xpos + CalPosOffsetX - CalWidth) + 'px';
	...
	else
	{
		...
		if (bNewCal === true)
		{
			winCal.style.left = (xpos + CalPosOffsetX - CalWidth) + 'px';
